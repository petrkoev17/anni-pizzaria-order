...
public class JwtReqFilter extends OncePerRequestFilter {
    ...
    private final transient JwtTokenVerify jwtTokenVerifier;

    @Autowired
    public JwtReqFilter(JwtTokenVerify jwtTokenVerifier) {
            ...
            if (directives.length == 2 && directives[0].equals(AUTHORIZATION_AUTH_SCHEME)) {
                ...
                try {
                    if (jwtTokenVerifier.validateToken(token)) {
                        String netId = jwtTokenVerifier.getUserIdFromToken(token);
                        var authenticationToken = new UsernamePasswordAuthenticationToken(
                                netId,
                                // no credentials and no authorities
                                null, List.of(jwtTokenVerifier.getUserRoleFromToken(token))
                        );
